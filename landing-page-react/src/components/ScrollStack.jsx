...
export const ScrollStackItem = ({ children, itemClassName = "" }) => (
  ...
  <div
    className={`scroll-stack-card relative w-10/12 sm:w-4/6 mx-auto h-auto sm:h-80 my-6 sm:my-8 p-6 sm:p-12 rounded-[20px] sm:rounded-[40px] shadow-[0_0_30px_rgba(0,0,0,0.1)] box-border origin-top will-change-transform ${itemClassName}`.trim()}
    style={{
...
const ScrollStack = ({
  ...
  const offsetCacheRef = useRef({ cardTops: [], endTop: 0, height: 0 });
  ...
  const computeOffsets = useCallback(() => {
    ...
    if (endEl) {
      if (useWindowScroll) {
        ...
        endTop = r.top + window.scrollY;
      } else {
        endTop = endEl.offsetTop;
      }
    }
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    const { cardTops, endTop } = offsetCacheRef.current;
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      const newTransform = {
        translateY, // float precision is fine; GPU handles sub-pixel smoothly
        scale,
        rotation,
        blur,
      };
